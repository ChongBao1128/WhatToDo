...
export default function EventDialog({ isOpen, event, onClose, onSave, onDelete, setEvent }: EventDialogProps) {
  return (
        ...
        <div className="p-4 space-y-3">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setEvent((prev) => (prev ? { ...prev, category: e.target.value } : null))}
              className="mt-1 block w-full border rounded p-1"
            />
          </div>
        </div>
        ...
        <DialogFooter className="flex justify-end gap-2 pe-4">
          {event && (event.action === "add" || event.id === "") ? (
            ...
            <>
              ...
              <Button variant="destructive" className="w-24 cursor-pointer" onClick={onDelete}>
                Remove
              </Button>
            </>
          )}

          <Button className="w-24 bg-neutral-400  cursor-pointer" onClick={onClose}>
            Close
          </Button>
        </DialogFooter>
      </DialogContent>
